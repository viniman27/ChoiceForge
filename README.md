...
- **CodeMirror source editor**: drop into any scene as raw ChoiceScript, then convert back to the visual graph. Body editors show inline `:initial-value` chips next to `${var}` references so you can proofread without translating identifiers in your head.
