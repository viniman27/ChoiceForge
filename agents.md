...
- **Variable casing check** (`src/domain/choicescript.ts::lintVariableCasing`): new pass runs after `lintUnusedVariables`. Builds canonical-by-lowercase map from `project.variables`, then scans every reference site (passage body, prompt, *if/*elseif/*selectable_if/*set/sets/*input_*/*rand inputs, options, fakeOptions, branches, and preserved sourceText). Emits `name_casing_inconsistent` warning per (canonical, variant, scene, node|line) triple with both names in params. **Gotcha encountered**: the existing `isValidChoiceScriptIdentifier` regex is strictly lowercase (`/^[a-z_][a-z0-9_]*$/`) — caught variants like `Coragem` as "not an identifier" and silently skipped them. Added a local case-insensitive helper `looksLikeIdentifier` inside `lintVariableCasing` to keep the new check working without changing the strict validator (which is correct elsewhere). 6 tests added.
- **Graphviz `.dot` export** (`src/domain/graphvizExport.ts`): new pure module. `exportProjectAsDot(project, { sceneName? })` emits a `digraph` with one cluster per non-special scene, rendering nodes with type-coloured fill+stroke (sRGB hex approximations of the editor's OKLCH palette) and edges styled by kind (`flow/choice/if/elseif/else` solid, `goto` dashed). Cross-scene edges added separately for `*goto_scene`/`*gosub_scene` targets, pointed at the first node of the target cluster. Node IDs prefixed with scene name to avoid collisions across clusters. Label format: `[type] title\nbody-snippet-up-to-60-chars`. Two helpers (`escapeDot`/`quoteDot`) handle DOT string quoting including embedded newlines. 6 tests added.
- **TopBar wiring**: new `onExportDot` prop, new "Grafo .dot" / ".dot graph" / "Grafo .dot" button next to Export. App.tsx adds `downloadGraphvizDot(project)` which uses `nativeSaveBytes` on Tauri or `<a download>` on web (mirrors existing zip download path). New `exportDot` field in `I18nLabels` + sample translations for PT/EN/ES.
- **Tests**: 387 + 12 new domain = **399 domain**, 99 UI = **498 total passing**. `tsc --noEmit` clean. Build clean.
- **Branch**: all on `dev` (will preview at `dev.choiceforge.pages.dev`); not merged to main yet — let it bake on dev, then cut v0.6.0.
- **Next**: commit + push to dev. Wait for visual preview, then decide if more goes into v0.6.0 before tagging.
...
- **v0.2.0 release prep: crash fix, Gatekeeper docs, opt-in update check, version bump.**
  ...
  - **macOS Gatekeeper documented properly** (`README.md`, `README.pt-BR.md`, `.github/workflows/desktop-release.yml`, v0.1.0 release notes via `gh release edit`): user reported that right-click → Open on macOS 15+ is NOT enough — Gatekeeper says "Apple could not verify this app is free of malware" and blocks regardless. The actual unblock paths are System Settings → Privacy & Security → "Open Anyway" after the block dialog appears, OR `xattr -dr com.apple.quarantine /Applications/ChoiceForge.app` in Terminal. README install sections rewritten in EN+PT. The release-notes template in the desktop-release workflow gets the same expanded instructions so every future tag ships them. The already-published v0.1.0 release notes were edited in place via `gh release edit --notes-file` so users finding the page today see the right workaround.
  - **Opt-in update check** (`src/platform/updateCheck.ts`, `App.tsx`, `styles.css`, `vite.config.ts`, `vitest.config.ts`): on launch the app fetches GitHub Releases API (`/releases/latest`), compares the latest tag against the running version via a semver-ish `isNewer` helper, and shows a top-of-screen banner with "View release" / "Later" / "Turn off" when a newer release exists. Result cached 6 h in localStorage. No signing infrastructure required (would have meant Tauri updater plugin + key generation + CI secrets); this is just a fetch + open-in-browser. `__APP_VERSION__` is injected by Vite from `package.json` at build time (and Vitest at test time) so bumping the version flows automatically. Three persistence keys: `choiceforge.updateCheck.v1` (cache), `choiceforge.updateCheck.dismissed` (last dismissed version), `choiceforge.updateCheck.optout` ('1' if Turn off clicked). Tests/ui/setup.ts sets opt-out by default so other UI tests don't fetch.
  - **Version bumped to 0.2.0** in `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`. README badges + counts updated (`tests-476_passing`, 89 UI tests).
  - **CHANGELOG**: new `[0.2.0]` section with Added (autosave/dirty/update-check/HelpGuide/Node Types), Changed (lint hint), Fixed (CodeMirror crash), Documentation (Gatekeeper workaround), Internal (test count + __APP_VERSION__).
  - **Tests**: 387 domain + 89 UI = **476 passing**. Build clean.
  - **Commits**: `c99b8ee` (CodeMirror fix), `e25c774` (Gatekeeper docs), `f167a28` (update check), this commit (version bump + CHANGELOG + agents).
  - **Next step**: tag `v0.2.0` and push → workflow drafts the release with new installers.
...
- **First public release: README rewrite, desktop release pipeline, Dependabot triage, v0.1.0 tag cut.**
  - **Dependabot triage** (7 open PRs reviewed):
    - Closed 3 major-version bumps with explanation: `typescript 5.9→6.0` (#7, waiting on @types/react + Vitest catch-up), `vite 4.5→8.0` (#6, four majors ahead — needs coordinated migration), `@vitejs/plugin-react 4.7→5.2` (#5, depends on Vite 5+).
    - Accepted 4 safe bumps: `@codemirror/view 6.42.1→6.43.0` (#3, minor), `react group 19.2.5→19.2.6` (#4, patch), `actions/setup-node v4→v6` (#1) and `actions/checkout v4→v6` (#2). The two GitHub Actions bumps were applied via local edit + close-with-comment because `gh` OAuth lacks `workflow` scope for `.github/workflows/*` merges.
  - **Desktop release pipeline** (`.github/workflows/desktop-release.yml`):
    - Tag-triggered (`v*`) GitHub Actions workflow, matrix builds on `macos-latest` (universal Intel+Apple Silicon) and `windows-latest`, uses `tauri-apps/tauri-action@v0` to run `cargo tauri build` and attach `.dmg`/`.msi` installers to a draft GitHub Release.
    - Tauri scaffold completed: app icons generated by rendering `public/favicon.svg` to 1024×1024 PNG via `qlmanage` and then running `npm run tauri:icons` (creates 32/64/128/256/512 PNGs + `icon.icns` + `icon.ico` + Square*Logo for Microsoft Store + Android mipmap + iOS AppIcon).
    - `tauri.conf.json` bundle section now lists explicit `targets` (dmg/msi/nsis/deb/appimage), `category: DeveloperTool`, `shortDescription`, `longDescription`, `homepage: https://choiceforge.pages.dev`, and the icon list. Without these the bundlers fail.
  - **README rewrite** (EN + PT-BR):
    - Hero with three big CTAs (Try in browser → choiceforge.pages.dev, Download desktop → Releases page, language switch) + CI/tests/license badges.
    - "What is it?" opening paragraph explains the pain (juggling `*goto` labels, `*if` chains, `*scene_list` across many `.txt` files) and how the graph addresses it. Includes a "Think Twine for ChoiceScript" one-liner.
    - "Pick how you want to use it" table contrasts the three install paths (web / desktop / source) so first-time visitors land on the right one immediately.
    - Desktop App promoted to a first-class section with install instructions, what the desktop version adds over web, and how to cut your own release (push a `v*` tag).
    - "Architecture in 30 Seconds" replaces the previous long architecture section; full deep dive stays in `agents.md`.
    - "Self-Hosting" section replaces the Cloudflare-specific one — no provider name in the README itself. The actual deploy is on Cloudflare Pages but the README treats it as "any static host".
    - Tech-stack table dropped — implementation detail, not a CTA.
    - `docs/cloudflare-pages.md` → `docs/deploying.md`: generalised host-agnostic deploy instructions + common-gotchas section.
  - **v0.1.0 tag** pushed (`git tag v0.1.0 && git push --tags`). Desktop release workflow triggered automatically; first draft GitHub Release with mac + win installers should appear in ~30-45 minutes.
  - **CHANGELOG.md**: new `[0.1.0]` section above `[Unreleased]` documenting the release.
  - **Tests**: 387 domain + 67 UI = **454 passing**. Build clean.
  - **Commits**: `22f4ad7` (Dependabot triage), `07378d9` (desktop release workflow + Tauri icons + bundle metadata), `bb2427e` (README + docs/deploying), v0.1.0 tag.
  - **Out-of-scope**: code signing for the desktop installers (would require Apple Developer + Authenticode certs and signing secrets). First-launch security prompts on mac/win are called out in the README and the GitHub Release notes.
