...
| Testes | Test runner nativo do Node (`node --test`) para domínio — 387 testes; Vitest + Testing Library para UI — 67 testes |
...
- **UI** (`tests/ui/`) — componentes React, o hook `useProjectStore`, cobertura de i18n, smoke test do `App`, error boundaries dos painéis e testes de round-trip export → reimport, via Vitest + jsdom + Testing Library. **67 passando.**
