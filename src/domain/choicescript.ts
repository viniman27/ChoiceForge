...
function lintUnusedVariables(project: ChoiceForgeProject, issues: LintIssue[]) {
  ...
  const scanNode = (node: StoryNode) => {
    ...
    if ((node.type === "rand" || node.type === "input_number") && node.inputMin && isValidChoiceScriptIdentifier(node.inputMin)) recordRead(node.inputMin);
    if ((node.type === "rand" || node.type === "input_number") && node.inputMax && isValidChoiceScriptIdentifier(node.inputMax)) recordRead(node.inputMax);
...
const AUDIO_EXTENSIONS = new Set(["mp3", "ogg", "wav", "aac", "m4a", "mp4"]);
...
export function computeVariableUses(project: ChoiceForgeProject): Map<string, number> {
  ...
  const scanNode = (node: StoryNode) => {
    ...
    if ((node.type === "rand" || node.type === "input_number") && node.inputMin && isValidChoiceScriptIdentifier(node.inputMin)) tally(node.inputMin);
    if ((node.type === "rand" || node.type === "input_number") && node.inputMax && isValidChoiceScriptIdentifier(node.inputMax)) tally(node.inputMax);
...
export function computeVariableLocations(project: ChoiceForgeProject): Map<string, VarLocation[]> {
  ...
  const scanGraph = (sceneName: string, nodes: StoryNode[]) => {
    for (const node of nodes) {
      ...
      if ((node.type === "rand" || node.type === "input_number") && node.inputMin && isValidChoiceScriptIdentifier(node.inputMin)) addLoc(node.inputMin, sceneName, node.id, node.title, "read");
      if ((node.type === "rand" || node.type === "input_number") && node.inputMax && isValidChoiceScriptIdentifier(node.inputMax)) addLoc(node.inputMax, sceneName, node.id, node.title, "read");
